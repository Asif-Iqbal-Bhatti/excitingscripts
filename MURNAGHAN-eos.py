...
def ameos(v,p):
    ...
    vv = (v0/v)**(2./3)
    ff = vv - 1.
    return e0 + 9.*b0*v0/16. * ( ff**3*bp + ( 6.-4.*vv )*ff**2 )   
...
def bmeos(v,p):
    ...
    bb = 1./(bp-1)
    vv = v0/v
    return e0 + b0/bp*v0/vv*(bb*pow(vv,bp) +1.)-bb*b0*v0    
...
with open('energy-vs-volume',"r") as input_energy:
